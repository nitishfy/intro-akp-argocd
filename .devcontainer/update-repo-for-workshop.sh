...
## Search for nitishfy/intro-akp-argocd and replace it with 
find ${workspace} -name '*.yaml' -type f -exec grep -l 'nitishfy/intro-akp-argocd' {} \; | while read file
do
    sed -i "s?nitishfy/intro-akp-argocd?${GITHUB_REPOSITORY}?g" ${file}
done
